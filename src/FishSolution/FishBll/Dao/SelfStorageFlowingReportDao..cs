...
namespace FishBll.Dao
{
    public class SelfStorageFlowingReportDao
    {
        ...
        protected FishEntity.SelfStorageFlowingReportVo DataRowToVo(DataRow row)
        {
            ...
            {
                vo.sgs_amine = int.Parse(row["sgs_amine"].ToString());
            }
            ...
            if (row["state"] != null)
            {
                vo.state = row["state"].ToString();
                //查询出状态值对应的状态名称
                FishEntity.SystemDataType item = FishEntity.Variable.StateList.Find((i) => { return i.Code.Equals(vo.state); });
                if (item != null)
                {
